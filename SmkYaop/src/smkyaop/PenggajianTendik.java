 ...
 */
public class PenggajianTendik extends javax.swing.JFrame {

    /**
     * Creates new form PenggajianNon
     */
    
    private int sksreg = 0,totalsksreg= 0 ,sksindustri = 0,totalsksindustri = 0,tunjabatan = 0,tunwalas = 0,jmlTran = 0,tottransport = 0,jmlPiket = 0,totpiket = 0,tunmasakerja = 0,tunbpjskesehatan = 0,totpotongan;
    private boolean isgty = false,iswalas = false;
    private long totgaji = 0;
    public PenggajianTendik() {
        initComponents();
        generateId();
        loadData("");
    }
    ...
    private void generateId(){
        String str = "";
        String query = "SELECT idgajian FROM gajiantendik ORDER BY idgajian DESC";
        try{
            ...
            if(rs.next()){
                ...
                tfGaji.setText("PTD"+nol);
            }else{
                tfGaji.setText("PTD0001");
            }
    ...
    private void loadData(String keys){
        ...
        String cari = "";
        try{
            String query = "SELECT gajiantendik.*,pegawai.namapegawai FROM pegawai INNER JOIN gajiantendik ON "
                    + "pegawai.idpegawai = gajiantendik.idpegawai ";
            if(keys != ""){
    ...
    public void setFromPegawai(String id, boolean isgty, boolean iswalas,int tunjjabatan){
        tfIdPeg.setText(id);
    ...
    private SwingWorker<Boolean,Void> hitunggaji(String tgl1,String tgl2){
        return new SwingWorker<Boolean,Void>(){
            ...
            protected Boolean doInBackground() throws Exception {                
                ...
                try{
                    String query = "SELECT pegawai.idpegawai,mengajar.jenis, SUM(mengajar.sks) as sksT,SUM(mengajar.totalharian) "
                            + "tota FROM mengajar INNER JOIN pegawai ON pegawai.idpegawai = mengajar.idpegawai"
                            + " WHERE pegawai.idpegawai = ? AND (mengajar.tanggal >= '"+tgl1+"' AND mengajar.tanggal <= DATE_ADD('"+tgl2+"',INTERVAL 1 DAY )) "
                            + "GROUP BY mengajar.jenis,mengajar.idpegawai";
                     ps = Database.getConn().prepareStatement(query);
                    ps.setInt(1, Integer.parseInt(idpegawai));
                    rs = ps.executeQuery();

                    System.out.println("hitung mengajar");
                    while(rs.next()){
                        if(rs.getString("jenis").equalsIgnoreCase("Industri")){
                            sksindustri += rs.getInt(3);
                            totalsksindustri += rs.getInt("tota");
                        }else{
                            sksreg +=rs.getInt(3);
                            totalsksreg+=rs.getInt("tota");
                        }
                    }
                    System.out.println("Mengajar reguler : "+sksreg );
                            System.out.println("hitung absensi");                              
                    String querys = "SELECT * FROM absensi WHERE idpegawai = ? AND (absensi.tanggal >= '"+tgl1+"' AND absensi.tanggal <= DATE_ADD('"+tgl2+"',INTERVAL 1 DAY )) ";
                    PreparedStatement ps2 = Database.getConn().prepareStatement(querys);
                    ps2.setInt(1, Integer.parseInt(idpegawai));
                    ResultSet rs2 = ps2.executeQuery();
                    while(rs2.next()){
                        int statusmasuk = rs2.getInt(6);
                        java.util.Date tgl =   new java.util.Date(rs2.getDate(3).getTime());
                        long jammasuk = rs2.getTime(4).getTime();
                        long jampulang = rs2.getTime(5).getTime();
                    ...
                    if(isgty){
                        System.out.println("hitung piket");                        

                        String query2 = "SELECT * FROM piket WHERE idpegawai = ? AND (piket.tanggal >= '"+tgl1+"' AND piket.tanggal <= DATE_ADD('"+tgl2+"',INTERVAL 1 DAY ))";
                        PreparedStatement ps3 = Database.getConn().prepareStatement(query2);
                        ps3.setInt(1, Integer.parseInt(idpegawai));
                        ResultSet rs3 = ps3.executeQuery();
                        while(rs3.next()){
                            jmlPiket +=1;
                            totpiket += 1*50000;
                        }
                        System.out.println("hitung bpjs");                        
                        String query4 = "SELECT * FROM pegawaijabat WHERE idpegawai = ?";
                        PreparedStatement ps4 = Database.getConn().prepareStatement(query4);
                        ps4.setInt(1, Integer.parseInt(idpegawai));
                        ResultSet rs4 = ps4.executeQuery();
                        while(rs4.next()){
                            int maskerja = rs4.getInt("masajabat");
                            if(maskerja % 5 == 0){
                                tunmasakerja += (maskerja * 50000);
                            }
                        }
                        System.out.println("hitung bpjs");
                        String query5 = "SELECT * FROM bpjspegawai WHERE idpegawai = ?";
                        PreparedStatement ps5 = Database.getConn().prepareStatement(query2);
                        ps5.setInt(1, Integer.parseInt(idpegawai));
                        ResultSet rs5 = ps5.executeQuery();
                        while(rs5.next()){
                            int jumlah = rs5.getInt("jumlah");
                    ...
                    totgaji += totalsksindustri + totalsksreg +tottransport +tunbpjskesehatan + tunmasakerja+totpiket+tunjabatan+tunwalas ;
                    System.out.println("Hitung potongan");   
                    String query2 = "SELECT pegawai.namapegawai ,potongan.nominal FROM potongan,potonganpegawai,pegawai "
                        ...
                        PreparedStatement ps6 = Database.getConn().prepareStatement(query2);
                        ps6.setInt(1, Integer.parseInt(idpegawai));
                        ResultSet rs6 = ps6.executeQuery();
                        while(rs6.next()){
                            int jumlah = rs6.getInt("nominal");
                            totpotongan += jumlah;
                        }                                  
    ...
    private void initComponents() {
        ...
        setTitle("Penggajian Tenaga Pendidikan");
        ...
        jLabel1.setText("Penggajian Tenaga Pendidikan");
        ...
        jLabel7.setText("Masukkan nama/NIP Pegawai");

        jLabel16.setBackground(new java.awt.Color(20, 165, 255));
        jLabel16.setFont(new java.awt.Font("Tahoma", 0, 22)); // NOI18N
        jLabel16.setForeground(new java.awt.Color(25, 59, 104));
        jLabel16.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/caricari.png"))); // NOI18N
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        ...
        if(tanya == JOptionPane.YES_OPTION && !id.isEmpty() ){
            try{
                PreparedStatement ps = Database.getConn().prepareStatement("DELETE fROM gajiantendik WHERE idgajian = ?");
                ps.setString(1, id);
                int hasil = ps.executeUpdate();
                if(hasil > 0 ){
                    PreparedStatement ps2 = Database.getConn().prepareStatement("DELETE FROM detailtendik WHERE idgajian = ?");
                    ps2.setString(1, id);
    ...
    private void jLabel15MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel15MouseClicked
        // TODO add your handling code here:
        HelperTable.isFromGajiTen = true;
        HelperTable.gajiT = this;
    ...
    private void btnSave1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSave1ActionPerformed
        ...
        String query = "INSERT INTO gajiantendik VALUES(?,?,?,?,?,?,?)";
        try{
            PreparedStatement ps = Database.getConn().prepareStatement(query);
            ps.setString(1, id);
            ...
            if(rs > 0){
                String query2 = "INSERT INTO detailtendik VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                PreparedStatement ps2 = Database.getConn().prepareStatement(query2);
                ps2.setString(1, id);
                ps2.setInt(2,sksreg);
                ps2.setInt(3,totalsksreg);
                ps2.setInt(4,sksindustri);
                ps2.setInt(5,totalsksindustri);
                ps2.setInt(6,tunjabatan);
                ps2.setInt(7,tunwalas);
                ps2.setInt(8,jmlTran);
                ps2.setInt(9,tottransport);
                ps2.setInt(10,jmlPiket);
                ps2.setInt(11,totpiket);
                ps2.setInt(12,tunmasakerja);
                ps2.setInt(13,tunbpjskesehatan);
                ps2.setTimestamp(14, new java.sql.Timestamp(new java.util.Date().getTime()));
 ...
 // {"ID Penggajian","ID Pegawai","Nama Pegawai","Tanggal Periode","Total Gaji","Total Potongan","Dibayarkan"};
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(PenggajianTendik.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
